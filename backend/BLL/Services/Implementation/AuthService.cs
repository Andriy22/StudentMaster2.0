...
public class AuthService : IAuthService
{
    ...
    public AuthService(UserManager<User> userManager, IJWTService jWTService, IRepository<RefreshToken> refreshTokens,
        IConfiguration configuration, IRepository<User> userRepos)
    {
    ...
    {
        var user = await _userRepos.GetQueryable(x => x.Email.ToLower() == model.Email.ToLower()).Include(x => x.Img)
            .FirstOrDefaultAsync();
        ...
        if (!isLoggined) throw new CustomHttpException("Password is wrong");

        if (!user.EmailConfirmed) throw new CustomHttpException("Account not confirmed", HttpStatusCode.Forbidden);
    ...
    {
        var token = await _refreshTokens.GetQueryable(x => x.Token == model.RefreshToken).Include(x => x.User)
            .ThenInclude(x => x.Img).FirstOrDefaultAsync();
