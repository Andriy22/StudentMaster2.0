...
public class StudentService : IStudentService
{
    ...
    public StudentService(IRepository<User> userRepository, IGroupService groupService,
        IRepository<Subject> subjectRepository, IRepository<Attendance> attendanceRepository,
        IAttendanceService attendanceService, IRepository<Work> workRepository, IRepository<Grade> gradeRepository,
    ...
    public async Task<List<List<RegisterRowViewModel>>> GetRegisterDataAsync(string studentId, int subjectId,
        bool isExtended)
    {
        ...
        foreach (var item in _userRepository.GetQueryable(x => x.Id == studentId).ToList())
        {
            ...
            rowData.Items.Add(new RegisterItemViewModel
            {
                ...
                Title = "Відвідування",
                Value =
                    $"{attendence.Count}/{(await _attendanceService.GetAttendanceDaysAsync(group.Id, subjectId)).Count}"
            });
