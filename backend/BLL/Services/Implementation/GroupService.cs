...
namespace backend.BLL.Services.Implementation
{
    public class GroupService : IGroupService
    {
        ...
        public GroupService(IRepository<Group> groupRepository, IRepository<Subject> subjectRepository, IRepository<User> userRepository, IRepository<TeacherGroup> teacherGroupRepository, UserManager<User> userManager)
        {
        ...
        public async Task AddStudentToGroup(int groupId, string studentId)
        {
            ...
            if (group == null)
            {
                throw new CustomHttpException($"Group with id [{groupId}] not found!");
            }
        ...
        {
            var group = await _groupRepository.GetQueryable(x=>x.Id == groupId).Include(x=>x.Subjects).FirstOrDefaultAsync();
            if (group == null)
            {
                throw new CustomHttpException($"Group with id [{groupId}] not found!");
            }
        ...
        public async Task ChangeGroupStatus(int groupId)
        {
            ...
            if (group.IsDeleted)
            {
                if (await _groupRepository.GetSingleAsync(x=>x.Name.ToLower() == group.Name.ToLower() && x.Id != group.Id && !x.IsDeleted) != null) {
        ...
        {
            var group = await _groupRepository.GetQueryable(x => x.Name.ToLower() == groupName.ToLower()).FirstOrDefaultAsync();
            if (group != null)
            {
                throw new CustomHttpException("Group name [" + groupName + "] is already used!");
            }
        ...
        public async Task<GroupInfoVM> GetGroup(int groupId)
        {
            ...
            if (group == null)
            {
                throw new CustomHttpException($"Group with id [{groupId}] not found!", System.Net.HttpStatusCode.NotFound);
            }
        ...
        {
            var group = await _groupRepository.GetQueryable(x => x.Students.FirstOrDefault(x => x.Id == studentId) != null).Include(x => x.Students).FirstOrDefaultAsync();

            if (group == null)
            {
                throw new CustomHttpException("Group not found...", System.Net.HttpStatusCode.BadRequest);
            }
        ...
        public async Task<IEnumerable<UserShortInfoVM>> GetStudentsByGroupId(int groupId)
        {
            ...
            var group = (await _groupRepository.GetQueryable(x => x.Id == groupId).Include(x => x.Students).FirstOrDefaultAsync());
        ...
        public async Task<IEnumerable<UserShortInfoVM>> GetTeachersByGroupId(int groupId)
        {
            ...
            var group = await _groupRepository.GetQueryable(x => x.Id == groupId).Include(x => x.TeacherGroups).ThenInclude(x=>x.User).FirstOrDefaultAsync();
        ...
        public async Task RemoveTeacherFromGroup(int groupId, string teacherId)
        {
            ...
            var teacherGroup = _teacherGroupRepository.GetQueryable(x => x.GroupId == groupId && x.UserId == teacherId).Include(x=>x.User).Include(x=>x.Group).FirstOrDefault();

            if (teacherGroup == null)
            {
                throw new CustomHttpException($"Teacher is not in group!");
            }
