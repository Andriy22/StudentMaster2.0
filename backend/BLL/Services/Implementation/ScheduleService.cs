...
public class ScheduleService : IScheduleService
{
    ...
    {
        var schedule = await _scheduleRepository.GetQueryable(x => x.GroupId == groupId && x.DayId == dayId)
            .Include(x => x.Day).Include(x => x.ScheduleItems).ThenInclude(x => x.ScheduleItemType)
            .Include(x => x.ScheduleItems).ThenInclude(x => x.Subject).FirstOrDefaultAsync();
    ...
    {
        var schedule = await _scheduleRepository.GetQueryable(x => x.GroupId == groupId && x.DayId == dayId)
            .Include(x => x.ScheduleItems).FirstOrDefaultAsync();

        if (schedule == null) throw new CustomHttpException("Invalid day or group id");

        var types = await GetScheduleItemTypesAsync();

        if (types == null && types.Count == 0) throw new CustomHttpException("No lecture types found!");


        var subjects = await _groupService.GetGroupSubjects(groupId);

        if (subjects == null && subjects.ToList().Count == 0) throw new CustomHttpException("No group subjects found!");

        var itm = schedule.ScheduleItems.OrderByDescending(x => x.Position).FirstOrDefault();

        var pos = 0;

        if (itm != null) pos = itm.Position;
    ...
    public async Task RemoveItemAsync(int itemId)
    {
        ...
        if (item == null) throw new CustomHttpException("Item not found!");
    ...
    {
        var item = await _scheduleItemRepository.GetQueryable(x => x.Id == model.Id).Include(x => x.Schedule)
            .FirstOrDefaultAsync();
        ...
        if (type == null) throw new CustomHttpException("No lecture type found!");


        var subject =
            (await _groupService.GetGroupSubjects(item.Schedule.GroupId)).FirstOrDefault(x => x.Id == model.SubjectId);

        if (subject == null) throw new CustomHttpException("No group subject found!");
