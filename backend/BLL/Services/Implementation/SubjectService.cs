...
public class SubjectService : ISubjectService
{
    ...
    public async Task AddSubjectToTeacher(int subjectId, string teacherId)
    {
        ...
        if (user == null) throw new CustomHttpException($"Teacher with id [{teacherId}] not found!");

        if (!await _userManager.IsInRoleAsync(user, "Teacher"))
            throw new CustomHttpException("User is not the teacher");

        var subject = await _subjectRepo.GetQueryable(x => x.Id == subjectId).Include(x => x.Teachers)
            .FirstOrDefaultAsync();
        if (subject == null) throw new CustomHttpException($"Subject with id [{subjectId}] not found!");

        if (subject.Teachers.Contains(user)) return;
    ...
    {
        if (string.IsNullOrEmpty(subjectName)) throw new CustomHttpException("Subject name can't be empty!");

        var subject = await _subjectRepo.GetQueryable(x => x.Name.ToLower() == subjectName.ToLower())
            .FirstOrDefaultAsync();
        if (subject != null) throw new CustomHttpException($"Subject with name [{subjectName}] already exists!");
    ...
    {
        var subjects = await _subjectRepo.GetQueryable(x => x.Teachers.FirstOrDefault(x => x.Id == teacherId) != null)
            .Include(x => x.Teachers).ToListAsync();
