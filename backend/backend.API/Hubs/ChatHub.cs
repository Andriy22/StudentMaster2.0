...
namespace backend.API.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        private readonly IRepository<DAL.Entities.ChatMessage> _chatRepository;
        ...
        {
            var user = await _userRepository.GetQueryable(x=>x.Id == Context.User.Identity.Name).Include(x=>x.Img).FirstOrDefaultAsync();
        ...
        {
            if (category < 1)
                category = _userRepository.GetQueryable(x => x.Id == Context.User.Identity.Name).Include(x => x.Group).FirstOrDefault().Group.Id;
        ...
        public async Task SendMessage(string message, int category)
        {
            ...
            var sender = await _userRepository.GetQueryable(x=>x.Id == Context.User.Identity.Name).Include(x=>x.Img).FirstOrDefaultAsync();
            var role = await GetRoleAsync(Context.User.Identity.Name);
            var color = "black";
            if (role == "Admin")
                color = "darkred";
        ...
        public void GetAllMessageWithUser(string uid)
        {
            ...
            foreach (var el in _chatRepository.GetQueryable(x => x.SenderId == your_uid && x.OwnerId == uid || x.OwnerId == your_uid && x.SenderId == uid).Include(x => x.Sender).ThenInclude(x=>x.Img))
        ...
        public async Task SendAllMessages(int category)
        {
            ...
            var msgs = _chatRepository.GetQueryable(x => x.Room == category.ToString()).Include(x => x.Sender).ThenInclude(x=>x.Img);
            ...
            await this.Clients.Group(category.ToString()).SendAsync("receiveAllMessages", result);
            result.Clear();
        }
        [Authorize]
        public async Task GetMyContactsAsync()
        {
            ...
            var msgs = _chatRepository.GetQueryable(x => (x.SenderId == Context.User.Identity.Name || x.OwnerId == Context.User.Identity.Name) && (x.Room == null || x.Room == "")).Include(x => x.Owner).Include(x => x.Sender).ThenInclude(x=>x.Img).ToList();
            ...
            foreach (var el in users)
                result.Add(new Contact() { Id = el.Id, FullName = $"{el.FirstName} {el.Name} {el.LastName}", Url = el.Img.Path });
