...
public class TeacherService : ITeacherService
{
    ...
    public async Task AddMarkAsync(string studentId, int criteriaId, int mark, string teacherId)
    {
        ...
        if (student is null) throw new CustomHttpException($"Student with id [{studentId}] not found");

        if (criteria is null) throw new CustomHttpException($"Column with id [{criteriaId}] not found");
        ...
        if (subjects is null) throw new CustomHttpException("Your subjects not found");
        ...
        if (group is null) throw new CustomHttpException("Student is not assigned to any group");

        if (group.Id != work.GroupId) throw new CustomHttpException("You don't have permission.");
    ...
    public async Task AddWorkAsync(CreateWorkDTO model)
    {
        ...
        if (group is null) throw new CustomHttpException($"Group with id [{model.GroupId}] not found");

        if (subject is null) throw new CustomHttpException($"Subject with id [{model.GroupId}] not found");
        ...
        bool isExtended, string teacherId)
    {
        ...
        if (teacher == null) throw new CustomHttpException("Invalid teacher id!");

        if (group == null) throw new CustomHttpException("Invalid group id");
    ...
    public async Task<List<ScheduleItemViewModel>> GetScheduleAsync(int dayId, string teacherId)
    {
        ...
        if (day == null) throw new CustomHttpException("Day not found");
        ...
        {
            if (!teacherGroups.Exists(x => x.Id == schedule.GroupId)) continue;
        ...
        DateTime date)
    {
        ...
        foreach (var student in students)
        {
            ...
            if (attendance != null) res.IsPresent = attendance.IsPresent;
    ...
    public async Task SetStudentAttendanceAsync(string studentId, int subjectId, DateTime date, bool attendance)
    {
        ...
        if (student == null) throw new CustomHttpException("Student not found");
